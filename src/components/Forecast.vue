		...
		weather[displayedForecast].length > groupSize
				...
				<div v-for="(day, i) in chunk" :key="i" class="day">
					...
					<div class="temperature">
						<div>
							...
							<span>
								{{ convertTemp(metric, day.temp.day) }}
							</span>
						</div>
						<div>
							...
							<span>
								{{ convertTemp(metric, day.temp.night) }}
							</span>
						</div>
					</div>
						...
						<Tooltip>
							<div class="tooltip">
								...
								<div>
									...
									<span>{{ convertSpeed(metric, day.wind_speed) }}</span>
								</div>
								<div>
								...
								<div>
									...
									<span>{{ convertTemp(metric, day.dew_point) }}</span>
								</div>
								<div>
								...
								<div>
									...
									<span>{{ new Date(day.sunrise * 1000).toLocaleString(undefined,
								...
								<div>
									...
									<span>{{ new Date(day.sunset * 1000).toLocaleString(undefined,
										{ hour: 'numeric', minute: 'numeric' }) }}</span>
								</div>
							</div>
						</Tooltip>
			...
			<div v-else-if="displayedForecast === 'hourly'" v-for="(chunk, indexD)  in splitIntoGroups(
				...
				<div v-for="(hour, i) in chunk" :key="i" class="hour">
					...
					<div>{{ convertTemp(metric, hour.temp) }}</div>
						...
						<Tooltip>
							<div class="tooltip">
								...
								<div>
									...
									<span>{{ convertSpeed(metric, hour.wind_speed) }}</span>
								</div>
								<div>
									<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
										<g>
								...
								<div>
									...
									<span>{{ convertTemp(metric, hour.dew_point) }}</span>
								</div>
								<div>
									<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512.013 512.013">
										<g>
