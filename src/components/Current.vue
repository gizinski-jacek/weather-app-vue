...
<template>
	<div v-if="weather && location">
		<div v-if="weather.alerts" class="alert" :class="{ show: showAlert }">
			<svg :onClick="toggleAlert" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
				<g>
					<path
			...
			<div>
				<div class="alert-content">
					<div>{{ weather.alerts.sender_name }}</div>
					<div>{{ weather.alerts.event }}</div>
					<div>{{ weather.alerts.description }}</div>
					<div>{{ weather.alerts.start }}</div>
					<div>{{ weather.alerts.end }}</div>
					<div>{{ weather.alerts.tags }}</div>
				</div>
			</div>
		</div>
